...
export function VipHubClient() {
  ...
  useEffect(() => {
    const refresh = () => { void loadMembershipTier().then(setTier); };
    refresh();
    void loadVipCouncilVote().then(setVote);
    window.addEventListener("barca-membership-change", refresh);
    return () => window.removeEventListener("barca-membership-change", refresh);
  }, []);
