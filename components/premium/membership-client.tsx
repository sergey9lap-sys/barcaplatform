...
import { MEMBERSHIP_KEY, membershipTiers, type MembershipTier } from "@/lib/premium/local";

export function MembershipClient() {
  const [active, setActive] = useState<MembershipTier>("free");
  useEffect(() => setActive((localStorage.getItem(MEMBERSHIP_KEY) as MembershipTier | null) ?? "free"), []);
  function activate(tier: MembershipTier) { localStorage.setItem(MEMBERSHIP_KEY, tier); setActive(tier); window.dispatchEvent(new Event("barca-membership-change")); }
  return <div className="space-y-5">
    ...
    <div className="flex items-start gap-3 rounded-2xl border border-amber-300/15 bg-amber-300/[0.05] p-4"><Badge variant="accent">Test mode</Badge><p className="ui-note text-sm">Сейчас кнопки только включают тариф локально. Деньги не списываются, платёжная система не подключена. Все игровые механики и XP остаются бесплатными.</p></div>
  </div>;
}
